...
float emptyTankDistance = 246;     // Distância medida pelo sensor quando o tanque está vazio (em cm).
float fullTankDistance =   38.2;      // Distância medida pelo sensor quando o tanque está cheio (em cm).
...
//--------------------------------------------------------------------------------------------------------------------------------------------------
const char* firmwareURL = "https://eduardocorrea62.github.io/CONDOMINIO_ART_DE_VIVER/Bloco_A_CISTERNA.ino.bin";
...
//     Blynk.virtualWrite(V16, String(rssi) + "db");  // Envia para o Blynk no Virtual Pin V14
...
BLYNK_WRITE(V23) { // Substitua V1 pelo pino virtual configurado no Blynk
  ...
  if (buttonState == 1) {
    ...
    enviarmensagemWiFi("CISTERNA BL-A: Botão acionado no Blynk WEB. Atualizando firmware do ESP32. Aguarde...");
...
void conectarWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    Serial.println("\nConectado ao Wi-Fi!");
    enviarmensagemWiFi("REDE Wi-Fi: Cisterna BL-A conectada à rede " + String(ssid));
  } else {
    Serial.println("\nFalha ao conectar ao Wi-Fi.");
    }
}
...
void arduinoOTA() {                                                             //ATUALIZAÇÃO DO CÓDIGO DO ESP32 VIA WI-FI ATRAVÉS DO OTA(Over-the-Air)
  ...
  ArduinoOTA.setHostname("CISTERNA_BL_A");                                 // Define o nome do dispositivo para identificação no processo OTA.
...
//-----------------------------------------------------------------------------------------------------------------------------------------
//Envia a Blynk os valores do nível e a distâmcia do sensor a superfície.
void atualizarDados() {
...
float calcularDistancia() {
  ...
  minDistancia = max(minDistancia, 38.2f);  // Adicionamos 'f' para garantir que seja float
  maxDistancia = min(maxDistancia, 246.0f); // Adicionamos 'f' para garantir que seja float
...
float leituraSimples() {
  ...
  float minDistancia = max(ultimaLeitura - TOLERANCIA, 38.2f);  // Adicionamos 'f' para indicar float
  float maxDistancia = min(ultimaLeitura + TOLERANCIA, 246.0f); // Adicionamos 'f' para indicar float
...
void checkWaterLevel() {
    ...
    if (nivelPercentual < (7 - HISTERESIS) && flag05) {
        sendMessage("CISTERNA DO BL-A: VAZIA");
    ...
    if (nivelPercentual > (97.5 + HISTERESIS) && flag100) {
        sendMessage("CISTERNA DO BL-A: CHEIA.\nNível praticamente em 100%");
        flag100 = false;
...
void blynkVirtualWrite(){
        // ALERTA SOBRE A BÓIA
    if (distancia <= (30 - HISTERESIS) && boiaNormal && (millis() - lastMessageTime >= messageInterval)) {
        sendMessage("BÓIA DA CISTERNA DO BL-A: \nProblemas na bóia, fazer inspeção.");
        lastMessageTime = millis();
        boiaNormal = false; // Define que já foi enviado o alerta
        Blynk.virtualWrite(V12, "Problemas na bóia, fazer inspeção.");
        Blynk.setProperty(V12, "color", "#FF0000"); // Vermelho
    }
    if (distancia > (31 + HISTERESIS) && !boiaNormal) {

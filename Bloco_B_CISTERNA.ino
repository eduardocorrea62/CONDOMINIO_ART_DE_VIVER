...
void sonarBegin(byte trig ,byte echo) { // Função para configurar o sensor ultrassônico (HC-SR04) e inicializar os pinos de disparo e recepção.
  ...
  #define intervaloMedida 100          // Intervalo entre medições (em milissegundos)
...
//     }
    ...
    //     sendMessage("CISTERNA DO BL-B:\nAbastecimento sendo restabelecido.\nNível acima de 40%");
    //     flag50 = false;
    // } else if (nivelPercentual < (41 - HISTERESIS) && !flag50) {
    //     sendMessage("CISTERNA DO BL-B:\nATENÇÃO: Nível a 38%");
    //     flag50 = true;
    // }
  // Alerta para nível de 50%
    if (nivelPercentual > (50 + HISTERESIS) && flag75) {
        //sendMessage("CISTERNA DO BL-B:\nAbastecimento sendo restabelecido.\nNível acima de 65%");
        flag75 = false;
    } else if (nivelPercentual < (52 - HISTERESIS) && !flag75) {
...
void blynkVirtualWrite(){
    //ALERTA SOBRE A BÓIA
    if (distancia <= (37 - HISTERESIS) && boiaNormal && (millis() - lastMessageTime >= messageInterval)) {
    ...
    if (distancia > (38 + HISTERESIS) && !boiaNormal) {

...
float emptyTankDistance = 259;    // Distância medida pelo sensor quando o tanque está vazio (em cm).
float fullTankDistance =  49;    // Distância medida pelo sensor quando o tanque está cheio (em cm).
...
float nivelPercentual_V5 = 0;    // Nível lido localmente no ESP-A (V4)
//-----------------------------------------------------------------------------------------------------------------------------------------
...
void setup() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        //Mensagem da reinicialização do ESP-32 para o Telegram
          enviarmensagemWiFi("ESP-32 da Cisterna B foi reinicializado");
        } else {
...
void conectarWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    Serial.println("\nConectado ao Wi-Fi!");
    enviarmensagemWiFi("REDE Wi-Fi: Cisterna BL B conectada à rede prioritária");
  } else {
...
void arduinoOTA() {                                                              //ATUALIZAÇÃO DO CÓDIGO DO ESP32 VIA WI-FI ATRAVÉS DO OTA(Over-the-Air)
 //ATUALIZAÇÃO DO CÓDIGO DO ESP32 VIA WI-FI ATRAVÉS DO OTA(Over-the-Air)
  ArduinoOTA.setHostname("CISTERNA_DO_BLOCO_B");                                 // Define o nome do dispositivo para identificação no processo OTA.
...
void sonarBegin(byte trig ,byte echo) { // Função para configurar o sensor ultrassônico (HC-SR04) e inicializar os pinos de disparo e recepção.
  ...
  #define intervaloMedida 5             // Intervalo entre medições (em milissegundos)

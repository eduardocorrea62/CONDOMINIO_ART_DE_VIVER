...
float fullTankDistance =   60;    // Distância medida pelo sensor quando o tanque está cheio (em cm).
...
void setup() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        //Mensagem da reinicialização do ESP-32 para o Telegram
          enviarmensagemWiFi("ESP-32 da Caixa BL B foi reinicializado");
        } else {
...
void conectarWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    Serial.println("\nConectado ao Wi-Fi!");
    enviarmensagemWiFi("REDE Wi-Fi: Caixa BL B conectada à rede prioritária");
  } else {
...
void arduinoOTA(){
  ...
  ArduinoOTA.setHostname("CAIXA_DO_BLOCO_B");                                     // Define o nome do dispositivo para identificação no processo OTA.
...
// Alerta para níveis 
 // Nível de 7%
    if (nivelPercentual < (7 - HISTERESIS) && flag05) {
        sendMessage("CAIXA DO BLOCO B: VAZIA");
  ...
  // Alerta para nível de 100%
    if (nivelPercentual > (97 + HISTERESIS) && flag100) {
        sendMessage("CAIXA DO BLOCO B: CHEIA.\nNível em 100%");
        flag100 = false;

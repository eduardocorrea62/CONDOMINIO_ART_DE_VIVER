...
const int totalChats = sizeof(chatIds) / sizeof(chatIds[0]);
  //Lista de IDs
  //Eduardo - "207223980" //Prof Tiago - "975571557"  //Amábile - "5255435232"
...
float fullTankDistance =  36;      // Distância medida pelo sensor quando o tanque está cheio (em cm).
...
float nivelPercentual_V1 = 0;    // Nível lido localmente no ESP-A (V4)
//--------------------------------------------------------------------------------------------------------------------------------------------------
...
void setup() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        //Mensagem da reinicialização do ESP-32 para o Telegram
          enviarmensagemWiFi("ESP-32 da Cisterna A foi reinicializado");
        } else {
...
void conectarWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    Serial.println("\nConectado ao Wi-Fi!");
    enviarmensagemWiFi("REDE Wi-Fi: Cisterna BL A conectada à rede prioritária");
  } else {
...
void arduinoOTA() {                                                             //ATUALIZAÇÃO DO CÓDIGO DO ESP32 VIA WI-FI ATRAVÉS DO OTA(Over-the-Air)
  ...
  ArduinoOTA.setHostname("CISTERNA_DO_BLOCO_A");                                 // Define o nome do dispositivo para identificação no processo OTA.
...
void displayData(float nivelPercentual) {
    static float lastValue = -1;  // Armazena o último valor do nível para evitar atualização desnecessária
    if (nivelPercentual != lastValue) { // Verifica se o valor do nível mudou
        ...
        lastValue = nivelPercentual;  // Atualiza o valor do nível armazenado
    }
}
//-----------------------------------------------------------------------------------------------------------------------------------------
...
void checkWaterLevel() {
    ...
    if (nivelPercentual > (25 + HISTERESIS) && flag25) {
        Serial.println("Condição de mensagem atendida: Nível acima de 25%");
        sendMessage("CISTERNA DO BL-A:\nAbastecimento sendo restabelecido.\nNível acima de 25%");
        flag25 = false;
    } else if (nivelPercentual < (25 - HISTERESIS) && !flag25) {
        Serial.println("Condição de mensagem atendida: Nível abaixo de 25%");
    ...
    if (distancia > (28 + HISTERESIS) && !boiaNormal) {
        Serial.println("Condição de mensagem atendida: Bóia operando normalmente");
